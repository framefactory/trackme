...
#define TUKEY_FUNC LM_ADD_PREFIX(tukey)
#define TUKEY_C ((LM_REAL)10.0)
inline LM_REAL TUKEY_FUNC(LM_REAL* e, LM_REAL* x, LM_REAL* hx, int n)
{
	int i;
	LM_REAL d, ad, cc, cc6, t0, t1;
	LM_REAL sum = 0;
	...
	if (x)
	{
		for (i = 0; i < n; i++)
		{
			...
			if (ad <= TUKEY_C) {
				t0 = ((LM_REAL)1.0) - d*d / cc;
				t1 = ((LM_REAL)1.0) - t0 * t0 * t0;
				sum += cc6 * t1;
			}
			else
				sum += cc6;
...
int LEVMAR_DER(
  ...
  int estimator,     /* 0: use L2-norm, 1: use tukey */
  ...
  printf("-- errors %.9g %0.9g\n", jacTe_inf, p_eL2);
   ...
   /* compute initial damping factor */
    ...
    /* compute p's new estimate and ||Dp||^2 */
	  ...
	  if (estimator)
		  pDp_eL2=TUKEY_FUNC(wrk2, x, wrk, n);
	  else
	  {
#if 1
